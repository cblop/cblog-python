...
class AbstractPageHandler(request.BlogRequestHandler):
    ...
    def augment_articles(self, articles, url_prefix, html=True):
        ...
        for article in articles:
            ...
            if html:
                try:
                    article.html = unescape(article.body)
                except AttributeError:
                    article.html = ''
            '''
            #article.path = '/' + defs.ARTICLE_URL_PATH + '/%s' % article.slug
            article.path = '/' + str(article.date_published.year) + '/' + str(article.date_published.month) +'/%s' % article.slug
    ...
    def render_articles(self,
        ...
        self.augment_articles(recent, url_prefix, html=True)

        last_updated = datetime.datetime.now()
        if articles:
            last_updated = articles[0].date_published
...
class ShortURLHandler(AbstractPageHandler):
    """
    Handles requests to display a single article, given its unique ID.
    Handles nonexistent IDs.
    """
    def get(self, short_url):
        article = Article.get_short_url(short_url)
        if article:
            self.redirect("/"+ str(article.date_published.year) + "/" + str(article.date_published.month) + "/" +article.slug)
        else:
            template = 'not-found.html'
            articles = []
            self.response.out.write(self.render_articles(articles=articles,
...
application = webapp.WSGIApplication(
    [('/', FrontPageHandler),
     ...
     ('/(\d\d\d\d)/(\d\d*)/?$', ArticlesForMonthHandler),
     ('/\d\d\d\d/\d\d*/([^/]+)/?$', SingleArticleHandler),
     ('/([^/]+)/?$', ShortURLHandler),
